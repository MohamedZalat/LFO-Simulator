% example: [a,b] = evaluateLevel2OrderKAgent('StraightLineAgent',7,1,8,10,2)

function [randomacc,learnedacc] = evaluateLevel2OrderKAgent(name,prefix,nmaps,CSIZE,XSIZE,EMIterations,ORDER)
...
for testtrace = 1:nmaps
	...
	for i = 1:nmaps
		if i ~= testtrace
                        % THIS ASSUMES ORDER = 2!!!!!!!!!!!!!!!
                        tmp = load([prefix traces{i}]);
                        % tmp = tmp(1:100,:);
                        tmp2 = ones(size(tmp,1),XSIZE*ORDER+1*ORDER);
                        tmp2(2:size(tmp,1),1:XSIZE+1) = tmp(1:(size(tmp,1)-1),CSIZE+1:CSIZE+XSIZE+1);
                        tmp2(:,XSIZE+2:XSIZE+1+XSIZE+1) = tmp(:,CSIZE+1:CSIZE+XSIZE+1);
			data = [data ; tmp2];
		else
                        tmp = load([prefix traces{i}]);
                        % tmp = tmp(1:100,:);
	...
	VARS = XSIZE + 1 + XSIZE + 1;       % get rid of the C
