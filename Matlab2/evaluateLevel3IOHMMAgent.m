...
%                        end
	...
	for x = 1:XSIZE
	  intra(x+CSIZE,1+CSIZE+XSIZE) = 1;
	end
	for c = 1:CSIZE
	  intra(c,1+CSIZE+XSIZE) = 1;
	end
...
%		disp(['Evaluating time step (all) ' num2str(i) ' -> ' num2str(prediction2) ' == ' num2str(testdata(i,CSIZE+XSIZE+1)) ' ? -> ' num2str(correct2) '/' num2str(total) ' = ' num2str(correct2/total)]);
	end
