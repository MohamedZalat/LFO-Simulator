...
public class BNetRemoteForce {
    ...
    public BNetRemoteForce(List<String> traces, int a_XSIZE, int a_YSIZE) {
        index = nextIndex++;
        XSIZE = a_XSIZE;
        YSIZE = a_YSIZE;
...
//        System.out.println("BNetRemove.run( " + input + " )");
        try {
            ...
            proxy.eval("tmp = zeros(" + YSIZE + ",1);\n");
            for(int i = 0;i<YSIZE;i++) {
                proxy.eval("tmpgauss = marginal_nodes(tmpeng, " + (XSIZE+i+1) + ");");
                proxy.eval("tmp(" + (i+1) + ") = sample_gaussian(tmpgauss.mu,tmpgauss.Sigma);");
            }
    ...
    public static void main(String args[]) {
        ...
        BNetRemoteForce net = new BNetRemoteForce(traces,10,2);
